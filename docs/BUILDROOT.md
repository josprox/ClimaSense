...
> Estado: ruta historica. La instalacion soportada usa Raspberry Pi OS y `scripts/build-raspios-bundle.sh`. Este documento se conserva para reproducir la imagen anterior, no para nuevas instalaciones.
...
La configuracion incluye el firmware/NVRAM `brcmfmac-sdio-firmware-rpi` para el Wi-Fi integrado del Zero 2 W, ademas de `brcmfmac`, `i2c-dev` y los buses BCM2835. Este camino no resolvio el fallo fisico del radio integrado del equipo probado; Raspberry Pi OS con adaptador USB es la configuracion validada.
