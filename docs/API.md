...
- `POST /api/v1/auth/login`, `POST /api/v1/auth/register` y `POST /api/v1/auth/logout`
...
- `POST /api/v1/tenant/activation-codes`
...
`GET /api/v1/auth/ws-token` exige una sesion valida y entrega el JWT de la cookie HTTP-only exclusivamente para mantenerlo en memoria durante la conexion. El navegador lo manda como primer mensaje a `WS /ws/dashboard`. Tras responder `ready`, el servidor publica eventos `refresh` segmentados por rol, usuario y empresa. El mensaje solo invalida la vista; los datos se vuelven a consultar mediante la API autorizada.
...
Los errores usan codigos HTTP reales mediante `ApiResponse`: `400` para telemetria invalida, `401` para autenticacion rechazada, `404` para ausencia de datos, `409` para activacion o correo duplicado y `503` cuando el servicio no esta listo. Altas y activaciones exitosas devuelven `201`. Las respuestas de token WebSocket usan `Cache-Control: no-store, private`.
